...
if (!role || !company || !city || !deadline || !href) {
  fail("role, company, city, deadline and link are all required.");
}
...
const id = `${slugify(company)}-${slugify(role)}-${deadline.slice(0, 7)}`;
...
file.items.sort((a, b) => a.deadline.localeCompare(b.deadline));
file.updatedAt = new Date().toISOString();
await fs.writeFile(FILE, `${JSON.stringify(file, null, 2)}\n`);

const open = file.items.filter((i) => Date.parse(`${i.deadline}T23:59:59Z`) >= Date.now()).length;
console.log(`
