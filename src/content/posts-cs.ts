...
const databaseDesignContentFr = [
  ...
  "Commencez par la question des index, celle dont les étudiants débattent sans données. Construisez une table de 200 000 lignes en mémoire et interrogez-la sur une colonne sans index : sqlite affiche SCAN chunks dans EXPLAIN QUERY PLAN et met environ 8,1 ms par requête, en moyenne sur 50 exécutions. Ajoutez CREATE INDEX i ON chunks(owner_id), lancez ANALYZE, et la même requête affiche SEARCH chunks USING COVERING INDEX i (owner_id=?) en environ 0,55 ms. Soit à peu près quinze fois plus rapide sur une table assez petite pour tenir en RAM — et c'est la ligne de plan qui vous dit laquelle des deux vous avez obtenue.",
...
const networkingBasicsContent = [
  ...
  "The same effect in your application code is one object. Measured over five requests each, requests.get(url) in a loop took about 226 ms per request; the identical calls through a single requests.Session() took about 60 ms. A client constructed inside your request handler is the loop version, and it is an easy mistake to make because it looks tidier.",
...
const networkingBasicsContentFr = [
  ...
  "Le même effet dans votre code applicatif tient en un objet. Mesuré sur cinq requêtes chacune, requests.get(url) dans une boucle prenait environ 226 ms par requête ; les mêmes appels via un unique requests.Session() prenaient environ 60 ms. Un client construit à l'intérieur de votre gestionnaire de requête, c'est la version en boucle — et l'erreur est facile à commettre, parce qu'elle a l'air plus propre.",
