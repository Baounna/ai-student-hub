...
export type BlogPost = {
  ...
  /**
   * Which references actually support which paragraph, by paragraph index.
   *
   * Every paragraph used to carry a numbered marker chosen by
   * (paragraphIndex % references.length) + 1. That is not attribution, it is
   * arithmetic: 253 paragraphs per locale, 506 in total, each telling a reader
   * "this specific claim is backed by source N" where nobody had ever compared
   * the claim to the source. A footnote is a promise, and a cycling one is a
   * promise generated by a counter.
   *
   * So markers are opt-in now. A paragraph gets one only when it appears here,
   * mapped to 1-based positions in `references`. Absent means no marker, and
   * the Sources list at the foot of the article -- which only ever claimed to be
   * the sources for the article as a whole -- carries the citation honestly.
   */
  paragraphCitations?: Record<number, number[]>;
...

  {
    ...
    locales: {
      en: {
        ...
        ,
          "Every figure in that paragraph came from one HTTP request, and you can repeat it in about three seconds. The date is in the URL on purpose: the file is a daily snapshot and the totals grow, so a percentage without a date drifts.",
          "```python\nimport csv, gzip, io, urllib.request\n\nDAY = \"2026-09-24\"\nurl = f\"https://epss.empiricalsecurity.com/epss_scores-{DAY}.csv.gz\"\nreq = urllib.request.Request(url, headers={\"User-Agent\": \"epss-check/1.0\"})\nwith urllib.request.urlopen(req, timeout=60) as r:\n    raw = gzip.decompress(r.read()).decode()\n\nlines = raw.splitlines()\nprint(lines[0])  # the file states its own model version and score date\nrows = csv.DictReader(io.StringIO(\"\\n\".join(lines[1:])))\nscores = [float(r[\"epss\"]) for r in rows]\nn = len(scores)\nprint(\"CVEs scored:\", n)\nfor t in (0.01, 0.1, 0.5):\n    below = sum(1 for s in scores if s < t)\n    print(\"below %-5s %7d = %5.2f%%   at or above: %6d = %5.2f%%\"\n          % (t, below, 100 * below / n, n - below, 100 * (n - below) / n))\n```"
        ]
      },
      fr: {
        ...
        ,
          "Tous les chiffres de ce paragraphe viennent d'une seule requête HTTP, et vous pouvez la refaire en trois secondes environ. La date est dans l'URL à dessein : le fichier est un instantané quotidien et les totaux augmentent, donc un pourcentage sans date dérive.",
          "```python\nimport csv, gzip, io, urllib.request\n\nDAY = \"2026-09-24\"\nurl = f\"https://epss.empiricalsecurity.com/epss_scores-{DAY}.csv.gz\"\nreq = urllib.request.Request(url, headers={\"User-Agent\": \"epss-check/1.0\"})\nwith urllib.request.urlopen(req, timeout=60) as r:\n    raw = gzip.decompress(r.read()).decode()\n\nlines = raw.splitlines()\nprint(lines[0])  # the file states its own model version and score date\nrows = csv.DictReader(io.StringIO(\"\\n\".join(lines[1:])))\nscores = [float(r[\"epss\"]) for r in rows]\nn = len(scores)\nprint(\"CVEs scored:\", n)\nfor t in (0.01, 0.1, 0.5):\n    below = sum(1 for s in scores if s < t)\n    print(\"below %-5s %7d = %5.2f%%   at or above: %6d = %5.2f%%\"\n          % (t, below, 100 * below / n, n - below, 100 * (n - below) / n))\n```"
        ]
      }
    },
